...
_logger = UnifiedLogger("silver_transforms")
...
def build_dim_ejecutora(
    ...
    from_ingreso = (
        ...
        .select(
            ...
            F.coalesce(F.col("EJECUTORA").cast("int"), F.col("SEC_EJEC").cast("int")).alias("EJECUTORA"),
    ...
    return (
        from_ingreso
        .unionByName(from_esat, allowMissingColumns=True)
...
def build_dim_ubigeo(
    ...
    return combined.dropDuplicates(["CODIGODEPARTAMENTO", "CODIGOPROVINCIA", "CODIGODISTRITO"])
...
def build_dim_nivel_gobierno(ingreso_df: DataFrame) -> DataFrame:
    return (
        ...
        .filter(F.col("NIVEL_GOBIERNO").isNotNull() & F.col("NIVEL_GOBIERNO_NOMBRE").isNotNull())
...
def build_dim_tipo_recurso(ingreso_df: DataFrame) -> DataFrame:
    return (
        ...
        .filter(F.col("TIPO_RECURSO").isNotNull() & F.col("TIPO_RECURSO_NOMBRE").isNotNull())
...
def build_dim_anio_aplicacion(rentas_ano_df: DataFrame) -> DataFrame:
    ...
    return (
        ...
        .select(
            ...
            F.coalesce(F.col("ANO_APLICACION_INICIO").cast("int"), F.lit(0)).alias("ANO_APLICACION_INICIO"),
            F.coalesce(F.col("ANO_APLICACION_FIN").cast("int"), F.lit(0)).alias("ANO_APLICACION_FIN"),
        )
...
def build_dim_pregunta_sismepre(
    ...
    return (
        ...
        .join(form_dedup.select("FORMULARIO_ID", "IdFormSismepre"), on="FORMULARIO_ID", how="inner")
        .drop("FORMULARIO_ID")
...
def build_dim_pregunta_renamu(renamu_dfs: list[DataFrame]) -> DataFrame:
    ...
    for rdf in renamu_dfs:
        ...
        all_parts.append(
            rdf.select(F.expr(stack_expr))
            .filter(F.col("NOMBRE_CAMPO").isNotNull())
        )

    if not all_parts:
        raise ValueError("No se encontraron columnas de preguntas en los archivos RENAMU")
    ...
    return (
        combined
        .select(
            F.substring(F.col("NOMBRE_CAMPO").cast("string"), 1, 100).alias("NOMBRE_CAMPO"),
            F.substring(
...
def build_fact_ingreso(
    ...
    generica_d = _dedup_by(dim_generica, ["GENERICA", "SUBGENERICA", "SUBGENERICA_DET"], "IdGenerica")
    especifica_d = _dedup_by(dim_especifica, ["ESPECIFICA", "ESPECIFICA_DET"], "IdEspecifica")

    return (
        ingreso_df
        .withColumn("IdTiempo", (F.col("ANO_DOC") * 100 + F.col("MES_DOC")).cast("int"))
        .filter(F.col("IdTiempo").isNotNull())
        .join(nivel_d.select("NIVEL_GOBIERNO", "IdNivelGobierno"), on="NIVEL_GOBIERNO", how="inner")
        .join(sector_d.select("SECTOR", "IdSector"), on="SECTOR", how="inner")
        .join(pliego_d.select("PLIEGO", "IdPliego"), on="PLIEGO", how="inner")
        .join(ejec_d.select("SEC_EJEC", "IdEjecutora"), on="SEC_EJEC", how="inner")
        .join(
            ubigeo_d.select("CODIGODEPARTAMENTO", "CODIGOPROVINCIA", "CODIGODISTRITO", "IdUbigeo"),
            on=[
        ...
        .join(tipo_d.select("TIPO_RECURSO", "IdTipoRecurso"), on="TIPO_RECURSO", how="inner")
        .join(
            generica_d.select("GENERICA", "SUBGENERICA", "SUBGENERICA_DET", "IdGenerica"),
        ...
        .select(
            ...
            F.coalesce(F.col("MONTO_PIA").cast("bigint"), F.lit(0).cast("bigint")).alias("MONTO_PIA"),
            F.coalesce(F.col("MONTO_PIM").cast("bigint"), F.lit(0).cast("bigint")).alias("MONTO_PIM"),
...
def build_fact_formulario_sismepre(
    ...
    preg_d = _dedup_by(dim_pregunta_sismepre, ["IdFormSismepre", "PREGUNTA_ID"], "IdPreguntaSismepre")

    return (
        respuestas_df
        .filter(F.col("PERIODO").isNotNull() & F.col("RESPUESTA_ID").isNotNull())
        .join(ejec_d.select("SEC_EJEC", "IdEjecutora"), on="SEC_EJEC", how="inner")
        .join(anio_d.select("ANO_APLICACION", "IdAnioAplicacion"), on="ANO_APLICACION", how="inner")
        .join(form_d.select("FORMULARIO_ID", "IdFormSismepre"), on="FORMULARIO_ID", how="inner")
        .join(
        ...
        .select(
            ...
            F.coalesce(F.col("RESPUESTA_ID").cast("smallint"), F.lit(0).cast("smallint")).alias("RESPUESTA_ID"),
...
def build_fact_renamu(
    ...
    for rdf in renamu_dfs:
        ...
        if year_col is None:
            _logger.warning("RENAMU DataFrame sin columna 'año', se omite en FACT_RENAMU")
            continue
        ...
        fixed_select = [
            ...
            if tipomuni_col else F.lit("").alias("TIPOMUNI"),
        ]
        ...
        unpivoted = (
            ...
            .withColumn("VALOR", F.substring(F.coalesce(F.col("VALOR"), F.lit("")), 1, 300))
    ...
    return (
        combined
        .filter(F.col("ANO").isNotNull() & F.col("CCDD").isNotNull())
        .join(
            ubigeo_d.select("CODIGODEPARTAMENTO", "CODIGOPROVINCIA", "CODIGODISTRITO", "IdUbigeo"),
            on=[
...
def build_facts(spark: SparkSession, stage: dict, server_dims: dict) -> dict[str, DataFrame]:
    """
