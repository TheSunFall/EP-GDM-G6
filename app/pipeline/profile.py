...
logger = UnifiedLogger("Profiler")

def run(args: argparse.Namespace) -> None:
    parquet_root = Path(args.parquet_dir) if getattr(args, "parquet_dir", None) else (
        settings.project_root / settings.config.api.path
    )
    report_dir = Path(args.report_dir) if getattr(args, "report_dir", None) else (
        settings.project_root / "data" / "profiling"
    )
    ...
    logger.info(f"Starting data quality profiling: {len(parquet_files)} files, output directory: {str(report_dir)}")
